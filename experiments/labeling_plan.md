...
> **2026-09-08 밤 라운드 3 재정의** — 실무 산업 CV / QC 관점으로 정의 명확화.
> 기존 정의는 winding_core 를 "심축+감긴 롤" 로 잡아 실제 winding action 없는 프레임까지 포함되던 문제 있었음.
> 재정의 원칙: **"공정 스테이션에서 CV 로 인식해 QC/이상감지에 쓸 대상"**.
> 각 클래스 라벨 예시: [`labeling_examples/`](labeling_examples/) 참조.

| 클래스 | 재정의 (GOOD 대상) | BAD (오라벨) | 실무 QC 시나리오 |
|---|---|---|---|
| **coating_die** | slot die head + slurry 접점 (head → foil 착점). GOOD 박스 = head + foil 접선 근방 (tangency) | 전체 코팅 라인, head 없는 foil 이송만, 이미 감긴 coated foil roll (post-coating), 자막/로고 | 슬러리 유량 이상 감지, 코팅 두께 편차, head 오염/막힘 감지 |
| **roll_press** (calendering) | 압연 롤 쌍 + 그 사이 필름 진입/배출 지점 (nip point). GOOD 박스 = 롤 쌍 (nip 포함) | 전체 라인, 롤 하나만, 다른 이송 롤(idler), 필름 없이 롤만 | 롤 압력 편차, 필름 두께 QC, 롤러 표면 마모/오염 감지 |
| **slitting_knife** | 회전 원반 나이프 어셈블리 (blades array + 축). GOOD 박스 = disc blade 배열 + 축 | 잘려나온 필름만, 전체 슬리팅 라인, 컨베이어 belt | 슬리팅 폭 정확도, 나이프 마모/파손, 슬리팅 위치 편차 |
| **winding_core** | **winding station** — 심축 + tangency point (필름 감기 접점) + 이송 필름. GOOD 박스 = 심축 + 접선 필름 접점 (다층 필름 이송 명확) | 완성된 jelly roll 정지 상태, 심축만 원경, post-winding assembly 라인, 셀 이송 | jelly roll 결함 감지, 필름 얼라인먼트 이상, 감기 속도 편차 |

**핵심 원칙 — "설비 자체의 윤곽에 밀착":** 배경·자막·판넬·완제품·라인 원경은 모두 배제. 라벨링 시 이 정의로 애매하면 **skip** (라벨 없는 프레임 = negative sample).
